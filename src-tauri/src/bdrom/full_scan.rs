...
mod tests {
    use super::*;
    use crate::protocol::{
        ChartSample, DiscInfo, PlaylistInfo, PlaylistStreamClipInfo, ScanProgressInfo,
    };
    use std::io::Cursor;
    ...
    fn pmt_payload() -> Vec<u8> {
        vec![
            0x00, 0x02, 0xB0, 0x1C, 0x00, 0x01, 0x01, 0x00, 0x00, 0xE0, 0x00, 0xF0, 0x00,
            0x1b, 0xF0, 0x11, 0xF0, 0x00, // AVC 0x1011
    ...
    fn build_m2ts_bytes() -> Vec<u8> {
        ...
        push(m2ts_atc(&ts_packet(true, 0x0000, &pat_payload(1, 0x0100)), atc), &mut data);
        atc += 27_000; // ~1ms
    ...
    fn publish_partial_file_snapshot_updates_progress_and_disc() {
        ...
        publish_partial_file_snapshot(
            &mut disc, &[0], "00001.M2TS", &progress, &base, &cached, &state, 1000,
        );
        let p = snapshot(&state);
    ...
    fn scan_one_file_populates_measurements() {
        ...
        scan_one_file(
            &bdrom, &entry.0, &mut disc, clip_name, &state, 0, &cached,
        )
    ...
    fn scan_one_file_cancel_discards_measurements() {
        ...
        let clip_name = "00001.M2TS";
        let entry = effective_stream_source(&bdrom, clip_name).expect("stream source");
        scan_one_file(&bdrom, &entry.0, &mut disc, clip_name, &state, 0, &cached)
            .expect("scan_one_file ok");
    ...
    fn scan_one_file_unknown_clip_is_noop() {
        ...
        scan_one_file(&bdrom, &entry.0, &mut disc, "99999.M2TS", &state, 0, &cached)
            .expect("noop ok");
