...
pub fn scan(
  ...
  loop {
    ...
    loop {
      ...
      if buffer.read_byte_default() == 0 && buffer.read_byte_default() == 0 && buffer.read_byte_default() == 1 {
        sync_byte_found = true;
        break;
      }
      buffer.bs_skip_bytes_default((stream_pos - buffer.position() as i64 + 1) as i32);
      if buffer.position() + 3 >= buffer.len() || (st.is_initialized && frame_type_read) {
        break;
      }
    }
    ...
    if buffer.position() < buffer.len() && sync_byte_found {
      ...
      buffer.bs_skip_bytes((last_stream_pos - buffer.position() as i64) as i32, true);
    }
  }
    ...
    if sps.profile_space == 0 {
      ...
      if sps.bit_depth_luma_minus8 == sps.bit_depth_chroma_minus8 {
        st.extended
          ...
          .push(format!("{} bits", sps.bit_depth_luma_minus8 as u32 + 8));
      }
      ...
      if sps.bit_depth_luma_minus8 + 8 == 10
        ...
        && (sps.vui_parameters.matrix_coefficients == 9 || sps.vui_parameters.matrix_coefficients == 10)
      ...
      if sps.vui_parameters.video_signal_type_present_flag {
        if st.extended_diagnostics {
          st.extended
            .extended_format_info
            .push(if sps.vui_parameters.video_full_range_flag == 1 {
...
fn slice_segment_layer(buffer: &mut TSStreamBuffer, nal_unit_type: i64, st: &State) -> bool {
  ...
  if !_first_slice_segment_in_pic_flag {
    if st.extended.pic_parameter_sets[slice_pic_parameter_set_id as usize].dependent_slice_segments_enabled_flag {
      let _dependent = buffer.read_bool(true);
    }
    return false;
  }
  buffer.bs_skip_bits_default(
    st.extended.pic_parameter_sets[slice_pic_parameter_set_id as usize].num_extra_slice_header_bits as u32,
  );
...
fn seq_parameter_set(buffer: &mut TSStreamBuffer, st: &mut State) {
  ...
  let from = if sps_sub_layer_ordering_info_present_flag {
    0
  } else {
...
fn sei_buffering_period(buffer: &mut TSStreamBuffer, seq_parameter_set_id: &mut u32, payload_size: u32, st: &State) {
...
fn sei_buffering_period_xxl(
  ...
  for _ in 0..xxl.sched_sel.len() {
    buffer.bs_skip_bits(xxl_common.initial_cpb_removal_delay_length_minus1 as u32 + 1, true);
    buffer.bs_skip_bits(xxl_common.initial_cpb_removal_delay_length_minus1 as u32 + 1, true);
...
fn sei_pic_timing(buffer: &mut TSStreamBuffer, seq_parameter_set_id: &mut u32, payload_size: u32, st: &State) {
  ...
  for _num_sub_layer in 0..=max_num_sub_layers_minus1 {
    ...
    if nal_hrd_parameters_present_flag || vcl_hrd_parameters_present_flag {
      *xxl_common = Some(XXLCommon {
        ...
        du_cpb_removal_delay_increment_length_minus1: du_cpb_removal_delay_increment_length_minus1 as u16,
        dpb_output_delay_du_length_minus1: dpb_output_delay_du_length_minus1 as u16,
        initial_cpb_removal_delay_length_minus1: initial_cpb_removal_delay_length_minus1 as u16,
...
fn sub_layer_hrd_parameters(
  ...
  for _ in 0..=cpb_cnt_minus1 {
    ...
    let bit_rate_value = ((bit_rate_value_minus1 as u64 + 1) * (1u64 << (6 + bit_rate_scale as u32))) as u64;
    let cpb_size_value_minus1 = buffer.read_exp(true);
    let cpb_size_value = ((cpb_size_value_minus1 as u64 + 1) * (1u64 << (4 + cpb_size_scale as u32))) as u64;
    if xxl_common.map(|x| x.sub_pic_hrd_params_present_flag).unwrap_or(false) {
      buffer.skip_exp(true);
      buffer.skip_exp(true);
    }
...
mod tests {
  ...
  fn extended_diagnostics_emits_mastering_and_light_level() {
    ...
    persistent.extended.mastering_display_color_primaries = "BT.2020".to_string();
    persistent.extended.mastering_display_luminance = "min: 0.0050 cd/m2, max: 1000 cd/m2".to_string();
  ...
  fn synthesized_vps_sps_is_parsed() {
    ...
    assert!(
      stream.encoding_profile.contains("Main 10"),
      "{}",
      stream.encoding_profile
    );
    assert!(
      stream.encoding_profile.contains("Level 4.1"),
      "{}",
      stream.encoding_profile
    );
