        ...
        for br in 0..16u32 {
          for sr in 0..4u32 {
            for ch in 0..4u32 {
              ...
              assert_eq!(
                stream.bit_rate,
                MPA_BITRATE[version as usize][layer as usize][br as usize] as u64 * 1000
              );
              assert_eq!(stream.sample_rate, MPA_SAMPLE_RATE[version as usize][sr as usize]);
              assert_eq!(stream.channel_count, MPA_CHANNELS[ch as usize]);
              assert_eq!(stream.audio_mode, MPA_CHANNEL_MODES[ch as usize].label());
              assert_eq!(
                stream.codec_name,
                format!("{} {}", MPA_VERSION[version as usize], MPA_LAYER[layer as usize])
              );
            }
          }
        }
