...
pub fn scan(stream: &mut TSStreamInfo, buffer: &mut TSStreamBuffer) {
  if stream.is_initialized && stream.core.as_ref().map(|c| c.is_initialized).unwrap_or(false) {
    return;
  }
  ...
  if ratebits != 0xF {
    stream.sample_rate = (if (ratebits & 8) > 0 { 44100u32 } else { 48000u32 }) << (ratebits & 7);
  }
