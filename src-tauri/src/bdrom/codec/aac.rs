...
const AAC_SAMPLE_RATES: [u32; 31] = [
  96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350, 0, 0, 57600, 51200, 40000,
  38400, 34150, 28800, 25600, 20000, 19200, 17075, 14400, 12800, 9600, 0, 0, 0,
];
      ...
      for profile in 0..4u32 {
        for sr_idx in 0..16u32 {
          for cm in 0..8u32 {
            ...
            assert_eq!(stream.audio_mode, AAC_CHANNEL_MODES[cm as usize].label(), "cm {cm}");
            assert_eq!(
              stream.codec_name,
