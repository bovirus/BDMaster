...
mod tests {
    ...
    fn pmt_payload() -> Vec<u8> {
        vec![
            0x00, 0x02, 0xB0, 0x17, 0x00, 0x01, 0x01, 0x00, 0x00, 0xE0, 0x00, 0xF0, 0x00,
            0x1b, 0xF0, 0x11, 0xF0, 0x00, // AVC PID 0x1011
    ...
    fn open_and_navigate_generated_udf_image() {
        let ran = with_generated_iso(|iso_path| {
            ...
            assert!(
                root_entries
                    .iter()
                    .any(|e| e.name.eq_ignore_ascii_case("BDMV")),
    ...
    fn scan_drives_iso_end_to_end() {
        let ran = with_generated_iso(|iso_path| {
            let disc = crate::bdrom::scan(&iso_path.to_string_lossy())
                .expect("scan succeeds on iso");
    ...
    fn build_type1_image() -> (TempPath, Vec<u8>, Vec<u8>) {
        ...
        b.lvd(259, 0, "OLDLABEL", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[type1_map(0)]);
        b.lvd(
            260,
            1, // higher vdsn -> this LVD wins
            "HANDBUILT",
            &LongAdSpec { length: 0, lbn: 0, prn: 0 }, // FSD at partition rel 0
            &[type1_map(0)],
        );
        b.td(261);
        ...
        b.fsd(p(0), &LongAdSpec { length: 0, lbn: 1, prn: 0 });
    ...
    fn handbuilt_lower_vdsn_descriptors_are_ignored() {
        ...
        b.lvd(259, 5, "WINNER", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[type1_map(0)]);
        b.lvd(260, 1, "STALE", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[type1_map(0)]);
        b.td(261);
        let p = |rel: u32| (PSTART + rel) as usize;
        b.fsd(p(0), &LongAdSpec { length: 0, lbn: 1, prn: 0 });
        b.fe_short(p(1), 4, SECTOR_SIZE as u64, &[(SECTOR_SIZE as u32, 2)]);
    ...
    fn handbuilt_bad_map_type_stops_parsing() {
        ...
        bad_map[0] = 7;
        bad_map[1] = 8;
        b.lvd(258, 0, "BADMAP", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[bad_map]);
        b.td(259);
        let p = |rel: u32| (PSTART + rel) as usize;
        b.fsd(p(0), &LongAdSpec { length: 0, lbn: 1, prn: 0 });
        b.fe_short(p(1), 4, SECTOR_SIZE as u64, &[(SECTOR_SIZE as u32, 2)]);
    ...
    fn handbuilt_no_partition_descriptor_errors() {
        ...
        b.avdp(257, 3);
        b.lvd(257, 0, "X", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[type1_map(0)]);
        b.td(258);
        let tmp = b.write_temp();
        let err = match UdfImage::open(&tmp.path) {
    ...
    fn handbuilt_no_lvd_errors() {
        ...
        assert!(
            err.to_string().contains("no Logical Volume Descriptor"),
            "got {}",
            err
        );
    }
    ...
    fn handbuilt_zero_maps_synthesizes_type1() {
        ...
        b.pd(257, 0, PSTART, 0);
        // LVD with zero maps.
        b.lvd(258, 0, "SYNTH", &LongAdSpec { length: 0, lbn: 0, prn: 0 }, &[]);
        b.td(259);
        let p = |rel: u32| (PSTART + rel) as usize;
        b.fsd(p(0), &LongAdSpec { length: 0, lbn: 1, prn: 0 });
        b.fe_short(p(1), 4, SECTOR_SIZE as u64, &[(SECTOR_SIZE as u32, 2)]);
    ...
    fn handbuilt_type2_other_map_falls_back_to_direct() {
        ...
        b.lvd(
            ...
            0,
            "OTHERMAP",
            &LongAdSpec { length: 0, lbn: 0, prn: 0 },
    ...
    fn handbuilt_metadata_partition_image() {
        ...
        b.lvd(
            ...
            0,
            "METADISC",
            &LongAdSpec { length: 0, lbn: 0, prn: 0 }, // FSD at metadata logical 0
        ...
        b.fsd(mlog(0), &LongAdSpec { length: 0, lbn: 1, prn: 0 });
    ...
    fn handbuilt_metadata_two_extents() {
        ...
        b.fe_short(
            ...
            5,
            (2 * ext_bytes) as u64,
            &[(ext_bytes, 2), (ext_bytes, 10)],
        );

        // Metadata logical layout.
        // extent1 (logical 0,1,2) at phys 282,283,284.
        b.fsd(phys(2), &LongAdSpec { length: 0, lbn: 1, prn: 0 }); // logical 0
    ...
    fn handbuilt_metadata_unknown_partition_errors() {
        ...
        assert!(
            err.to_string().contains("references unknown partition"),
            "got {}",
            err
        );
    }
    ...
    fn directory_size_breaks_cycles() {
        ...
        write_dir_sector(
            &mut b,
            p(2),
            &[(0x08, "", 1), (0x02, "SUB", 3), (0x00, "LEAF.BIN", 5)],
        );
