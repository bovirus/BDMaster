...
pub fn render_output_file_name(template: &str, file_name: &str, values: &StreamTemplateValues) -> String {
  let mut map: HashMap<&str, String> = HashMap::new();
  map.insert("file_name", file_name.to_string());
  map.insert("video_count", values.video_count.to_string());
  map.insert("video_codec_1", sanitize_file_name_part(&values.video_codec_1));
  map.insert("audio_count", values.audio_count.to_string());
  map.insert("audio_codec_1", sanitize_file_name_part(&values.audio_codec_1));
  map.insert("text_count", values.text_count.to_string());
  map.insert("text_codec_1", sanitize_file_name_part(&values.text_codec_1));
...
pub fn playlist_template_values(disc_path: &str, playlist_name: &str) -> Result<StreamTemplateValues> {
...
pub fn stream_template_values(disc_path: &str, stream_name: &str) -> Result<StreamTemplateValues> {
  let path = Path::new(disc_path);
  let use_ssif = crate::config::get_config().scan.enable_ssif_support;
  let bdrom = crate::bdrom::open_bdrom(path, use_ssif)?;
  let disc = crate::bdrom::to_disc_info(&bdrom);
...
fn stream_values_from_playlist(pl: &PlaylistInfo) -> StreamTemplateValues {
  let (text_count, text_codec_1) = subtitle_count_and_first_codec(&pl.graphics_streams, &pl.text_streams);
...
fn subtitle_count_and_first_codec(graphics_streams: &[TSStreamInfo], text_streams: &[TSStreamInfo]) -> (usize, String) {
...
mod tests {
  ...
  fn substitutes_all_known_placeholders() {
    ...
    assert_eq!(rendered, "00001-1xHEVC-2xDTS-HD MA-3xPGS");
  }
