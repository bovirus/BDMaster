...
async fn open_playlist_in_mkvtoolnix_gui(
  ...
  mkvtoolnix::spawn_mkvtoolnix_gui(&resolved, &disc_path, to_path.as_deref(), &values).map_err(convert_error)
}
...
async fn open_stream_file_in_mkvtoolnix_gui(
  ...
  let resolved = bdrom::resolve_stream_file_path(&disc_path, &stream_name).map_err(convert_error)?;
  let template = config::get_config().integration.mkv.output_file_template;
  let values = if template::template_needs_stream_values(&template) {
...
#[tauri::command]
async fn open_playlist_in_bettermediainfo(disc_path: String, playlist_name: String) -> Result<(), String> {
  let resolved = bdrom::resolve_playlist_path(&disc_path, &playlist_name).map_err(convert_error)?;
  bettermediainfo::spawn_bettermediainfo(&resolved.to_string_lossy()).map_err(convert_error)
}

#[tauri::command]
async fn open_stream_file_in_bettermediainfo(disc_path: String, stream_name: String) -> Result<(), String> {
  let resolved = bdrom::resolve_stream_file_path(&disc_path, &stream_name).map_err(convert_error)?;
  bettermediainfo::spawn_bettermediainfo(&resolved.to_string_lossy()).map_err(convert_error)
}
...
#[tauri::command]
async fn open_playlist_in_mpchc(disc_path: String, playlist_name: String) -> Result<(), String> {
  let resolved = bdrom::resolve_playlist_path(&disc_path, &playlist_name).map_err(convert_error)?;
  mpchc::spawn_mpchc(&resolved.to_string_lossy()).map_err(convert_error)
}

#[tauri::command]
async fn open_stream_file_in_mpchc(disc_path: String, stream_name: String) -> Result<(), String> {
  let resolved = bdrom::resolve_stream_file_path(&disc_path, &stream_name).map_err(convert_error)?;
  mpchc::spawn_mpchc(&resolved.to_string_lossy()).map_err(convert_error)
}
