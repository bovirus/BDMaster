...
pub async fn is_bettermediainfo_found(path: String, check_running: bool) -> Result<BetterMediaInfoStatus> {
  if check_running {
...
pub fn spawn_bettermediainfo(file: &str) -> Result<()> {
  ...
  let bin = binary_path(&dir).ok_or_else(|| anyhow::anyhow!("BETTERMEDIAINFO_NOT_AVAILABLE:{}", dir.display()))?;
