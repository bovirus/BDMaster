...
use crate::template::{StreamTemplateValues, render_output_file_name};
...
fn output_path(source_file: &Path, to_path: &str, template: &str, values: &StreamTemplateValues) -> Result<PathBuf> {
...
fn resolve_output_path(
  ...
  let to_path = to_path.map(str::trim).filter(|p| !p.is_empty()).unwrap_or(from_path);
...
fn write_gui_output_config(output: &Path) -> Result<PathBuf> {
  let output_dir = output
    ...
    .ok_or_else(|| anyhow::anyhow!("Output path has no parent directory: {}", output.display()))?;
  ...
  let file =
    File::create(&config_path).map_err(|_| anyhow::anyhow!("OUTPUT_DIR_NOT_WRITABLE:{}", output_dir.display()))?;
...
pub fn spawn_mkvtoolnix_gui(
  ...
  if !source_file.exists() {
    return Err(anyhow::anyhow!("Path {} does not exist.", source_file.display()));
  }
...
mod tests {
  ...
  fn output_path_falls_back_to_stem_when_template_renders_empty() {
    ...
    let output = output_path(&source_file, "output", "", &StreamTemplateValues::default()).unwrap();
