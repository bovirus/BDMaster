...
- Provides `UdfFileReader`, a streaming `Read` implementation over allocation extents or embedded allocation data without loading whole files.
...
- D-strings cover OSTA compression IDs 8 and 16; symlinks, file versions, permissions, timestamps, and extended attributes are intentionally ignored because they do not feed the Blu-ray scan DTOs.

## Open Issues

- This is not a direct `../BDInfo` implementation gap: BDInfo delegates the whole ISO filesystem to DiscUtils. Full general-purpose DiscUtils conformance (sparable/virtual partitions, integrity/defect handling, descriptor CRC validation, and filesystem metadata) remains outside BDMaster's Blu-ray/UHD reader scope.
